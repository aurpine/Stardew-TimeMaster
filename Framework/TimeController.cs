...
namespace TimeMaster.Framework
{
    internal class TimeController
    {
        ...
        internal TimeController(IModHelper helper, IMonitor monitor, IManifest manifest, ModConfig config)
        {
            ...
            helper.Events.GameLoop.DayStarted += (s, e) =>
            {
                ...
                AutoFreezeReason autoFreeze = Config.ShouldFreeze(Game1.timeOfDay, Game1.currentLocation);
                playerStates.UpdateTimeSpeedSettings(Game1.player.UniqueMultiplayerID, tickInterval: tickInterval, autoFreeze: autoFreeze);
            };
            ...
            helper.Events.Player.Warped += (s, e) =>
            {
                ...
                playerStates.UpdateTimeSpeedSettings(Game1.player.UniqueMultiplayerID, tickInterval: tickInterval, autoFreeze: autoFreeze, skipUpdate: true);
                if (Config.TimeFlowChangeNotifications)
                {
                    Monitor.Log($"Time speed settings for location '{e.NewLocation.Name}': {tickInterval / 1000f:N} seconds per 10 minutes", LogLevel.Debug);
                }
            };
        ...
        internal void ReloadConfig()
        {
            ...
            ClientSideState.Value.ShouldLockMonsters = Config.LockMonsters;
            Helper.Multiplayer.SendMessage(Config.LockMonsters, Messages.SetLockMonstersMode, new string[1] { ModManifest.UniqueID });
        ...
        private void Input_ButtonPressed(object? sender, StardewModdingAPI.Events.ButtonPressedEventArgs e)
        {
            ...
            else if (Config.Keys.FreezeTime.JustPressed())
            {
                ...
                if (freezeOverride)
                {
                    ...
                    Helper.Multiplayer.SendMessage("The host has paused.", Messages.VoteUpdateMessage, new[] { ModManifest.UniqueID });
                    if (Config.TimeFlowChangeNotifications)
                    {
                ...
                else
                {
                    ...
                    Helper.Multiplayer.SendMessage("The host has unpaused.", Messages.VoteUpdateMessage, new[] { ModManifest.UniqueID });
                    if (Config.TimeFlowChangeNotifications)
                    {
        ...
        private void Multiplayer_PeerConnected(object? sender, StardewModdingAPI.Events.PeerConnectedEventArgs e)
        {
            ...
            {
                Notifier.NotifyErrorInChatBox(I18n.Message_PlayerDoesNotHaveMod(Game1.getFarmer(e.Peer.PlayerID).Name, ModManifest.Name));
                Monitor.Log(I18n.Message_PlayerDoesNotHaveMod(Game1.getFarmer(e.Peer.PlayerID).Name, ModManifest.Name), LogLevel.Warn);
            }
            else if (!timeMasterMod.Version.Equals(ModManifest.Version))
            {
                Notifier.NotifyErrorInChatBox(I18n.Message_PlayerDoesNotHaveCorrectVersion(Game1.getFarmer(e.Peer.PlayerID).Name, ModManifest.Version));
                Monitor.Log(I18n.Message_PlayerDoesNotHaveCorrectVersion(Game1.getFarmer(e.Peer.PlayerID).Name, ModManifest.Version), LogLevel.Warn);
                Notifier.NotifyErrorInChatBox(I18n.Message_PlayerVersionComparison(ModManifest.Version, Game1.getFarmer(e.Peer.PlayerID).Name, timeMasterMod.Version));
                Monitor.Log(I18n.Message_PlayerVersionComparison(ModManifest.Version, Game1.getFarmer(e.Peer.PlayerID).Name, timeMasterMod.Version), LogLevel.Warn);
            }
        ...
        private void Multiplayer_ModMessageReceived(object? sender, StardewModdingAPI.Events.ModMessageReceivedEventArgs e)
        {
            ...
            try
            {
                switch (e.Type)
                {
                    ...
                    default:
                        Monitor.LogOnce($"Unable to handle message ({e.Type}). Sender: PlayerID={e.FromPlayerID}. Receiver IsMainPlayer={Context.IsMainPlayer}", LogLevel.Error);
                        break;
                }
            }
            catch (Exception ex)
            {
                Monitor.LogOnce($"Unable to handle message ({e.Type}). Sender: PlayerID={e.FromPlayerID}. Receiver IsMainPlayer={Context.IsMainPlayer}. Technical details:\n{ex}", LogLevel.Error);
            }
        }
        ...
        private void GameLoop_UpdateTicking(object? sender, StardewModdingAPI.Events.UpdateTickingEventArgs e)
        {
            ...
            if (clientSideState.LastEventState != Game1.eventUp)
            {
                ...
                {
                    Helper.Multiplayer.SendMessage(Game1.eventUp, Messages.UpdateEventState, new string[1] { ModManifest.UniqueID }, new long[1] { Game1.MasterPlayer.UniqueMultiplayerID });
                }
            }
            ...
            if (clientSideState.PauseIsRequested != clientRequestsPause)
            {
                ...
                {
                    Helper.Multiplayer.SendMessage(clientRequestsPause, Messages.UpdatePauseRequestState, new string[1] { ModManifest.UniqueID }, new long[1] { Game1.MasterPlayer.UniqueMultiplayerID });
                }
            }
            ...
            if (isTimeFrozen)
            {
                ...
                if (Game1.buffsDisplay.food != null && Game1.buffsDisplay.food.millisecondsDuration > clientSideState.FoodDuration)
                {
                    clientSideState.FoodDuration = Game1.buffsDisplay.food.millisecondsDuration;
                }
                if (Game1.buffsDisplay.food != null)
                {
                    Game1.buffsDisplay.food.millisecondsDuration = clientSideState.FoodDuration;
                }

                if (Game1.buffsDisplay.drink != null && Game1.buffsDisplay.drink.millisecondsDuration > clientSideState.DrinkDuration)
                {
                    clientSideState.DrinkDuration = Game1.buffsDisplay.drink.millisecondsDuration;
                }
                if (Game1.buffsDisplay.drink != null)
                {
                    Game1.buffsDisplay.drink.millisecondsDuration = clientSideState.DrinkDuration;
                }

                if (Game1.buffsDisplay.otherBuffs.Count > 0 && clientSideState.OtherBuffsDurations.Count == 0)
                {
        ...
        private void VotePauseToggle()
        {
            ...
            {
                Helper.Multiplayer.SendMessage(clientSideState.IsVoteForPauseAffirmative, Messages.UpdateVoteForPause, modIDs: new[] { ModManifest.UniqueID }, playerIDs: new[] { Game1.MasterPlayer.UniqueMultiplayerID });
            }
        }
